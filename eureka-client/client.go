...
func (c *Client) Start() *Client {
	...
	log.Debug("Register application instance successful\n")
	// 刷新服务列表
	go c.refresh()
	// 心跳
	go c.heartbeat()
	...
	for {
		if c.Running {
			if err := c.doRefresh(); err != nil {
				...
				log.Debug("Refresh application instance successful")
			}
		} else {
			break
		}
	...
	for {
		if c.Running {
			if err := c.doHeartbeat(); err != nil {
				c.doRegister() // 如果发现异常，重新触发注册一次
				log.Error(err.Error())
			} else {
				log.Debug("Heartbeat application instance successful")
			}
		} else {
			break
		}
	...
	for {
		...
		case syscall.SIGTERM:
			...
			if err != nil {
				...
				log.Debug("UnRegister application instance successful")
			}
			os.Exit(0)
		}
	}
